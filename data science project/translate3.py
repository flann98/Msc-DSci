...
def parse(file, output_dir, threshold):
    ...
    try:
        ...
        for model in structure:
            for chain in model:
                ...
                for residue in chain:
                    ...
                    add_edges = [];
                    for other_res_id, other_res in gcn_graph.nodes(data="residue"):
                        if other_res_id != res_node:
                            # Calculates euclidean distance between points.
                            dist = np.linalg.norm(np.array((gcn_graph.nodes[res_node]['coordX'],
                                                           gcn_graph.nodes[res_node]['coordY'],
                                                           gcn_graph.nodes[res_node]['coordZ'])) \
                                                  - np.array((gcn_graph.nodes[other_res_id]['coordX'],
                                                             gcn_graph.nodes[other_res_id]['coordY'],
                                                             gcn_graph.nodes[other_res_id]['coordZ'])));
        ...
        nx.write_graphml(gcn_graph,
                         fr'{output_dir}/{fileID}-gcn_graph.graphml');
        return;
...
def iterator(direct=r'dataset/AlphaFold Protein Database e.coli/Uncompressed',
             output_dir=r'dataset/AlphaFold Protein Database e.coli/Uncompressed/graphs/graphML', 
             threshold=6.0):
    """
